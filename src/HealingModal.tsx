...
const VibrationTool: React.FC = () => {
    ...
    const [questionFlow, setQuestionFlow] = useState<'challenges' | 'spiritual' | 'breakthroughs' | 'complete' | null>(null);
    ...
    const generateFinalPrescription = async () => {
        try {
            ...
            if (healingData.success) {
                ...
                setMessages((prev) => [...prev, {
                    sender: "ai",
                    text: healingData.message || "Your personalized healing prescription has been generated successfully!"
                }]);
                ...
                setMessages((prev) => [...prev, {
                    sender: "ai",
                    text: healingData.message || "Failed to generate prescription. Please try again."
                }]);
            }
        } catch (error) {
    ...
    const handleImageUpload = async () => {
        ...
        try {
            ...
            if (faceData.success && faceData.data) {
                ...
                setMessages((prev) => [...prev, { sender: "ai", text: faceData.message }]);
                await handleAstrologyAPI();
            } else {
                setMessages((prev) => [...prev, { sender: "ai", text: "Face analysis failed. Please try again." }]);
            }
        } catch (error) {
            console.error("Face API error:", error);
            setMessages((prev) => [...prev, { sender: "ai", text: "Error occurred during face analysis." }]);
        }
    };
    ...
    const handleAstrologyAPI = async () => {
        try {
            setMessages((prev) => [...prev, { sender: "ai", text: "Getting astrology data..." }]);
            ...
            setMessages((prev) => [...prev, { sender: "ai", text: "Error occurred during astrology analysis." }]);
        }
    };
        ...
        try {
            ...
            recorder.onstop = async () => {
                ...
                try {
                    ...
                    console.log("Converted audio blob:", convertedBlob.size, "bytes, type:", convertedBlob.type);
                    ...
                    console.error("Audio conversion failed, using original:", conversionError);
    ...
    const handleVoiceAnalysis = async (audioBlob: Blob, audioUrl: string) => {
        try {
            ...
            if (voiceData.success && voiceData.data) {
                ...
                setMessages((prev) => [...prev, { sender: "ai", text: voiceData.message }]);
                ...
                setMessages((prev) => [...prev, {
                    sender: "ai",
                    text: voiceData.message || "Voice analysis failed. Please try a longer recording with clear speech."
                }]);
            }
        } catch (error) {
    ...
    const handleVoiceAnalysisFormData = async (audioUrl: string) => {
        try {
            ...
            if (voiceData.success && voiceData.data) {
                ...
                setMessages((prev) => [...prev, {
                    sender: "ai",
                    text: voiceData.message || "Voice analysis failed. Please try again."
                }]);
            }
        } catch (error) {
            console.error("Voice API error:", error);
            setMessages((prev) => [...prev, { sender: "ai", text: "Error occurred during voice analysis." }]);
        }
    };
