...
const TarotCard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTarot = async () => {
      ...
      try {
        const response = await fetch(`${API_URL}/api/v1/numerology/tarot_reading`, {
          method: 'POST',
          headers: {
        ...
        if (result.success && result.data) {
          ...
          const formattedData: TarotReading = {
            card_backcover: apiData.card_backcover || "https://res.cloudinary.com/ds64xs2lp/image/upload/v1746909534/backcover_dgkxji.png",
            dob: apiData.dob || dob,
            mode: apiData.mode || "random",
            name: apiData.name || username,
            reading: apiData.reading?.map((card: any) => ({
  ...
  const getPositionColor = (position: string): string => {
    switch (position) {
      case 'past': return 'text-info';
      case 'present': return 'text-warning';
      case 'future': return 'text-success';
  ...
  return (
    <div className="min-vh-100 vw-100" style={{
      background: '#000000',
...
<div className="d-flex align-items-center justify-content-center position-relative w-100 mb-4" style={{ height: '80px' }}>
                  ...
                  >
                    ...
                    <div className="position-absolute top-0 start-0 w-100 p-3">
                      <div className="d-flex justify-content-between align-items-center">
                        <span className={`badge fs-6 ${getPositionColor(card.position)}`} style={{ background: 'rgba(0,0,0,0.7)' }}>
                          {getPositionIcon(card.position)} {card.position.toUpperCase()}
                        </span>
                        {card.reversed && (
                      ...
                      >
                        ...
                        >
                          ...
                          {!revealedCards[index] && (
                            <div className="position-absolute top-50 start-50 translate-middle">
                              <div className="text-warning" style={{ fontSize: '2rem', animation: 'pulse 2s infinite' }}>
                                ✨
                              </div>
                            </div>
                          )}
                        ...
                        <div
                          ...
                          style={{
                            backfaceVisibility: 'hidden',
                            transition: 'transform 0.8s ease',
                            transform: revealedCards[index] ? 'rotateY(0deg)' : 'rotateY(180deg)',
                            borderRadius: '15px',
                            overflow: 'hidden',
                            boxShadow: '0 10px 30px rgba(0,0,0,0.5)',
                          }}
                    ...
                    <div className="card-body pt-0">
                      {revealedCards[index] ? (
                        <>
                          ...
                          <div className="card-text text-light" style={{ lineHeight: '1.6' }}>
                        ...
                        <div className="text-center py-4">
                          ...
                          <p className="text-light mb-0">Click to reveal your {card.position} card</p>
                        </div>
                      )}
                    </div>
                  </div>
